...
//using namespace Qt;
ManoMeter900::ManoMeter900(QWidget *parent)
        : AbstractMeter900(parent)
{
        m_max=300.0;
        m_min=0.0;
...
void ManoMeter900::initCoordinateSystem(QPainter & painter)
{
...
void ManoMeter900::paintBackground(QPainter & painter)
{
	...
	if (true || digitOffset())
	  ...
	  for (int i=0;i<10;i++)
	  {
	    double v = m_min + i*(m_max - m_min)/9.0;
	    if (fabs(v) < 0.000001 ) v = 0.0;
        v = (v < 1000) ? v : v / 1000;
	    QString val = QString("%1").arg(v);
            QSize Size = painter.fontMetrics().size(Qt::TextSingleLine, val);
            painter.save();
	    painter.translate( digitOffset() * cos((5.5+i)*PI/6.667), digitOffset() * sin((5.5+i)*PI/6.667));
...
void ManoMeter900::paintEvent(QPaintEvent * )
{
	drawBackground();
